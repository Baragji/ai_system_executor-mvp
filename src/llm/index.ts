...
import { startLlmSpan } from "../telemetry/llmSpans.js";
...
export async function generateJSON(messages: LLMMessage[], options: GenerateJSONOptions = {}): Promise<string> {
  ...
  const span = await startLlmSpan("llm.generate", {
    ...
    "llm.model": process.env.LLM_MODEL || null,
    "llm.max_retries": maxRetries,
    "llm.max_tool_iterations": maxToolIterations
  });
  ...
  try {
    for (;;) {
      span.setAttributes({ "llm.attempt": attempt + 1 });

      const traceContext = getTraceContext();
      if (traceContext?.sessionId) {
        span.setAttributes({ "llm.session_id": traceContext.sessionId });
      }
      if (traceContext?.phase) {
        span.setAttributes({ "llm.phase": traceContext.phase });
      }
      ...
      try {
        ...
        for (let depth = 0; depth <= maxToolIterations; depth += 1) {
          ...
          if (response.toolCalls && response.toolCalls.length > 0 && tools.length > 0) {
            ...
            for (const call of response.toolCalls) {
              ...
              span.addEvent("llm.tool_call", { name: tool.name, callId: call.id });
              conversation.push({
                role: "tool",
                name: tool.name,
                toolCallId: call.id,
        ...
        span.setAttributes({
          "llm.result.status": "success",
          "llm.attempts": attempt + 1,
          "llm.tool.calls": executedTools.length,
          "llm.response.tool_calls": Boolean(response.toolCalls && response.toolCalls.length > 0)
        });
        ...
        span.addEvent("llm.retry", { attempt: attempt + 1, backoffMs: backoff, status, code, isTimeout });
    ...
    if (error instanceof PausedError) {
      span.setAttributes({ "llm.result.status": "paused" });
      spanMessage = error.message;
      spanStatus = "ok";
      throw error;
    }
    spanStatus = "error";
    spanMessage = error instanceof Error ? error.message : String(error);
    span.setAttributes({ "llm.result.status": "error" });
