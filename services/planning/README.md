# Executor Planning Service

A dedicated microservice that hosts planning workflows for the Executor platform. It ships with an Express + TypeScript runtime, OpenTelemetry bootstrap, and RFC 9457 problem details middleware following the shared service template.
...
Visit `http://localhost:3002/healthz` to verify the service is running.
...
Set `OTEL_ENABLED=1` (or `true`) to enable OpenTelemetry traces. Configure the collector endpoint via `OTEL_EXPORTER_OTLP_ENDPOINT`. Override the service resource name with `SERVICE_NAME` (defaults to `executor-planning-service`).
