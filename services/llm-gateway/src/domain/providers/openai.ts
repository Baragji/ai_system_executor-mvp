...
export class OpenAIProvider {
  ...
  async generate(messages: LLMMessage[], options: ProviderGenerateOptions = {}): Promise<ProviderGenerateResult> {
    const maxRetries = Math.max(0, Number(process.env.LLM_MAX_RETRIES ?? 2));
    const initialBackoff = Math.max(0, Number(process.env.LLM_INITIAL_BACKOFF_MS ?? 300));
    const maxBackoff = Math.max(initialBackoff, Number(process.env.LLM_MAX_BACKOFF_MS ?? 2000));
    ...
    for (;;) {
      try {
        ...
        if (!retryable || attempt >= maxRetries) {
          if (retryable && attempt >= maxRetries) {
            await logEvent("llm_retry_exhausted", { attempt: attempt + 1, status, code, message });
          }
          throw err;
        }
  ...
  private async generateOnce(messages: LLMMessage[], options: ProviderGenerateOptions = {}): Promise<ProviderGenerateResult> {
    ...
    if (process.env.LLM_PROVIDER_DEBUG) {
      try {
        await logEvent("llm_provider_debug_response_meta", {
          ...
          hasChoices:
            resp && typeof resp === "object" && Array.isArray((resp as unknown as { choices?: unknown[] }).choices),
      ...
      try {
        ...
        await logEvent("llm_provider_request_context", {
          ...
          tools: requestParams.tools
            ? (requestParams.tools as Array<{ function?: { name?: string } }>).map(t => t?.function?.name ?? "unknown")
            : [],
        });
