...
new file mode 100644
index 0000000000000000000000000000000000000000..8c6f60be44c199ea9f239756967eb723bd4d6270
--- /dev/null
...
+    "task_id": "WF3-W31",
+    "status": "succeeded",
+    "started_at": "2025-10-14T21:10:02.417Z",
+    "completed_at": "2025-10-14T21:42:38.903Z",
+    "elapsed_ms": 195146,
+    "notes": "PhaseState context memoized in runtime; cached workflow metadata now attaches to progress snapshots and SSE payloads."
+  }
+]
...
index c7d49f8057d1ab0aad652c00a5bcb0516667c0c9..e54a7bd9a88f166da2ef99a5e0da902c47eaf56b 100644
--- a/public/index.html
+++ b/public/index.html
@@ -4,50 +4,98 @@
     <meta charset="utf-8" />
...
+          <span id="workflowUpdatedAt" class="workflow-summary__timestamp">—</span>
+        </div>
+        <p id="workflowHumanSummary" class="workflow-summary__human">Workflow insights will appear after the first execution update.</p>
+        <div class="workflow-summary__grid">
+          <div class="workflow-summary__item">
+            <span class="label">Current gate</span>
+            <span id="workflowCurrentGate" class="value">—</span>
+          </div>
+          <div class="workflow-summary__item">
+            <span class="label">Next gate</span>
+            <span id="workflowNextGate" class="value">—</span>
+          </div>
+          <div class="workflow-summary__item">
+            <span class="label">Current task</span>
+            <span id="workflowCurrentTask" class="value">—</span>
+          </div>
+          <div class="workflow-summary__item">
+            <span class="label">Next task</span>
+            <span id="workflowNextTask" class="value">—</span>
+          </div>
+          <div class="workflow-summary__item">
+            <span class="label">Suggested action</span>
+            <span id="workflowSuggestedAction" class="value">—</span>
+          </div>
+          <div class="workflow-summary__item">
+            <span class="label">Suggested command</span>
+            <code id="workflowSuggestedCommand">—</code>
+          </div>
+        </div>
+        <div class="workflow-summary__meta">
+          <div>
+            <span class="label">Uncommitted changes</span>
+            <span id="workflowUncommitted" class="value">—</span>
+          </div>
+          <div>
+            <span class="label">Validations</span>
+            <span id="workflowValidations" class="value">—</span>
+          </div>
+        </div>
+        <div>
...
index d96054a042b5e0d8b58ce11c623c9b46f05fb8ed..a8c6c99eb9d18e8265390700bcfdd71ebc39fdf4 100644
--- a/public/script.js
+++ b/public/script.js
...
+const workflowUpdatedAt = document.getElementById("workflowUpdatedAt");
+const workflowCurrentGate = document.getElementById("workflowCurrentGate");
+const workflowNextGate = document.getElementById("workflowNextGate");
...
+const workflowSuggestedAction = document.getElementById("workflowSuggestedAction");
+const workflowSuggestedCommand = document.getElementById("workflowSuggestedCommand");
+const workflowUncommitted = document.getElementById("workflowUncommitted");
+const workflowValidations = document.getElementById("workflowValidations");
+const workflowPendingTasks = document.getElementById("workflowPendingTasks");
...
@@ -60,50 +72,154 @@ function clone(value) {
...
+  }
+  const normalizedStatus = gate.status.replace(/_/g, " ");
+  return `${gate.id} • ${normalizedStatus}`;
+}
+
...
+  }
+  const status = task.status ? task.status.replace(/_/g, " ") : "pending";
+  return `${task.id} • ${task.title || "Untitled"} (${status})`;
+}
+
+function clearWorkflowSummary() {
+  if (!workflowSummarySection) return;
+  workflowSummarySection.classList.add("hidden");
+  workflowHumanSummary && (workflowHumanSummary.textContent = "Workflow insights will appear after the first execution update.");
+  workflowUpdatedAt && (workflowUpdatedAt.textContent = "—");
+  workflowCurrentGate && (workflowCurrentGate.textContent = "—");
+  workflowNextGate && (workflowNextGate.textContent = "—");
+  workflowCurrentTask && (workflowCurrentTask.textContent = "—");
+  workflowNextTask && (workflowNextTask.textContent = "—");
+  workflowSuggestedAction && (workflowSuggestedAction.textContent = "—");
+  workflowSuggestedCommand && (workflowSuggestedCommand.textContent = "—");
+  workflowUncommitted && (workflowUncommitted.textContent = "—");
+  workflowValidations && (workflowValidations.textContent = "—");
+  if (workflowPendingTasks) {
+    workflowPendingTasks.innerHTML = "";
+  }
+}
+
+function renderWorkflowSummary(metadata) {
+  if (!workflowSummarySection) return;
+  if (!metadata) {
...
+  if (workflowHumanSummary) {
+    workflowHumanSummary.textContent = metadata.humanSummary || "";
+  }
+  if (workflowUpdatedAt) {
+    try {
+      workflowUpdatedAt.textContent = new Date(metadata.computedAt).toLocaleString();
+    } catch {
+      workflowUpdatedAt.textContent = "—";
+    }
+  }
+  if (workflowCurrentGate) {
+    workflowCurrentGate.textContent = describeGate(metadata.currentGate);
+  }
+  if (workflowNextGate) {
+    workflowNextGate.textContent = metadata.nextGate ? describeGate(metadata.nextGate) : "—";
+  }
+  if (workflowCurrentTask) {
+    workflowCurrentTask.textContent = describeTask(metadata.currentTask);
+  }
+  if (workflowNextTask) {
+    workflowNextTask.textContent = metadata.nextTask ? describeTask(metadata.nextTask) : "—";
+  }
+  if (workflowSuggestedAction) {
+    workflowSuggestedAction.textContent = metadata.suggestedNextAction?.action ?? "—";
+  }
+  if (workflowSuggestedCommand) {
+    workflowSuggestedCommand.textContent = metadata.suggestedNextAction?.command ?? "—";
+  }
+  if (workflowUncommitted) {
+    const count = Array.isArray(metadata.uncommittedChanges) ? metadata.uncommittedChanges.length : 0;
+    workflowUncommitted.textContent = count === 0 ? "Clean" : `${count} pending`;
+  }
+  if (workflowValidations) {
+    const v = metadata.validations;
+    workflowValidations.textContent = v
+      ? `lint=${v.lint} • type=${v.typecheck} • test=${v.test} • contract=${v.contract_check}`
+      : "Not run";
+  }
+  if (workflowPendingTasks) {
+    workflowPendingTasks.innerHTML = "";
+    const tasks = Array.isArray(metadata.pendingTasks) ? metadata.pendingTasks : [];
+    if (tasks.length === 0) {
+      const li = document.createElement("li");
+      li.textContent = "All tasks complete";
+      workflowPendingTasks.appendChild(li);
+    } else {
+      tasks.slice(0, 5).forEach(task => {
+        const li = document.createElement("li");
+        li.textContent = describeTask(task);
+        workflowPendingTasks.appendChild(li);
+      });
+      if (tasks.length > 5) {
+        const li = document.createElement("li");
+        li.textContent = `… and ${tasks.length - 5} more`;
+        workflowPendingTasks.appendChild(li);
+      }
+    }
+  }
...
@@ -251,50 +367,51 @@ async function handleResumeSubmit(event) {
   } catch (err) {
     if (resumeMessageEl) {
...
+  renderWorkflowSummary(snapshot?.workflowMetadata ?? null);
...
@@ -1386,50 +1503,51 @@ function collectClarificationAnswers() {
...
index b9b4faa148e2e54b5f15a6e1d05ccd9c3ecf1d2e..b1a0d0e54dfd7467fcb6e18274c323dd90a1b4c7 100644
--- a/public/styles.css
+++ b/public/styles.css
@@ -1,43 +1,56 @@
...
+.workflow-summary { margin-top: 24px; padding: 16px; border: 1px solid #334155; border-radius: 12px; background: rgba(15, 23, 42, 0.65); display: grid; gap: 16px; }
+.workflow-summary__header { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; }
+.workflow-summary__timestamp { font-size: 12px; color: #94a3b8; }
+.workflow-summary__human { margin: 0; font-size: 14px; color: #cbd5e1; }
+.workflow-summary__grid { display: grid; gap: 12px; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); }
+.workflow-summary__item { display: grid; gap: 4px; padding: 12px; border-radius: 10px; background: rgba(15, 23, 42, 0.55); border: 1px solid rgba(148, 163, 184, 0.15); }
+.workflow-summary__meta { display: grid; gap: 12px; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); }
+.workflow-summary__list { margin: 8px 0 0; padding-left: 18px; color: #cbd5e1; }
+.workflow-summary__list li { margin: 2px 0; }
+.workflow-summary .label { font-size: 12px; letter-spacing: 0.05em; text-transform: uppercase; color: #94a3b8; }
+.workflow-summary .value { font-size: 14px; font-weight: 500; color: #e2e8f0; }
+.workflow-summary code { background: rgba(15, 23, 42, 0.8); border-color: rgba(148, 163, 184, 0.25); padding: 4px 6px; font-size: 12px; display: inline-block; }
+
 ...
 #debugDisclosure summary::-webkit-details-marker { display: none; }
 
 #debugDisclosure summary:hover { color: #e2e8f0; }
diff --git a/src/server.ts b/src/server.ts
index 850a882bc9868844ba6cdb9888820cc676020157..5daf804bffe0cfb4b2bc21ddb4eac00c167b6cc8 100644
--- a/src/server.ts
+++ b/src/server.ts
@@ -1,35 +1,36 @@
...
+const workflowMetadataBySession = new Map<string, WorkflowMetadata>();
+
+async function ensureWorkflowMetadataForSession(sessionId: string): Promise<WorkflowMetadata> {
+  const existing = workflowMetadataBySession.get(sessionId);
+  if (existing) {
+    return existing;
+  }
+
...
+    uncommittedChanges
+  });
+  const metadata = buildWorkflowMetadata(phaseState, nextAction, {
+    validations,
+    uncommittedChanges
+  });
+  workflowMetadataBySession.set(sessionId, metadata);
+  return metadata;
+}
+
+function getWorkflowMetadata(sessionId: string): WorkflowMetadata | null {
+  return workflowMetadataBySession.get(sessionId) ?? null;
+}
+
+function clearWorkflowMetadata(sessionId: string): void {
+  workflowMetadataBySession.delete(sessionId);
+}
+
...
+  clearWorkflowMetadata(sessionId);
 ...
 function stateToStage(state: OrchestratorState): string {
   switch (state) {
     case "CLARIFYING":
       return "analyzing";
@@ -206,79 +263,83 @@ function purgeExpiredProgressSessions(now: number) {
     }
   }
 }
...
@@ -291,60 +352,62 @@ function normalizeInterruptQuestions(input: unknown): InterruptQuestionInput[] {
       ...(metadata ? { metadata } : {})
     });
   }
...
+    ...(workflowMetadata ? { workflowMetadata } : {})
 ...
 function openProgressStream(req: Request, res: Response, sessionId: string): void {
   ...
   const close = () => {
@@ -1531,50 +1594,56 @@ app.post("/api/clarify", (req, res) => {
...
@@ -1892,50 +1961,56 @@ app.post("/api/plan/:project/retest-subtask", async (req, res) => {
   } catch (err) {
...
@@ -1990,50 +2065,56 @@ app.post("/api/sessions/:id/pause", async (req, res) => {
...
@@ -2260,39 +2341,49 @@ app.get("/api/files/:project/:path(*)", async (req, res) => {
...
+  snapshot(sessionId: string) { return getProgress(sessionId); },
+  update(sessionId: string, stage: string, progress: number, data?: Record<string, unknown>, done?: boolean) {
+    setProgress(sessionId, stage, progress, data, done);
+  },
+  ensureWorkflow(sessionId: string) {
...
+    workflowMetadataBySession.clear();
+  }
 };
...
index 351d5cabb54c887c376e843f604b543639a78d1e..4d1929a817e4a797c0b925f75420e37f3ae02b6e 100644
--- a/src/state/phaseState.ts
+++ b/src/state/phaseState.ts
@@ -23,50 +23,64 @@ export interface PhaseState {
...
+  currentGate: { id: string; status: GateStatus } | null;
+  nextGate: { id: string; status: GateStatus } | null;
...
@@ -178,25 +192,122 @@ export function suggestNextAction(
...
+
+export function sortGates(gates: Record<string, GateStatus>): Array<[string, GateStatus]> {
+  return Object.entries(gates).sort((a, b) => a[0].localeCompare(b[0], undefined, { numeric: true, sensitivity: "base" }));
+}
+
+function deriveCurrentGate(gates: Record<string, GateStatus>): { id: string; status: GateStatus } | null {
...
+    return null;
+  }
+  const [lastId, lastStatus] = last;
+  return { id: lastId, status: lastStatus };
+}
+
+function deriveNextGate(gates: Record<string, GateStatus>): { id: string; status: GateStatus } | null {
+  const sorted = sortGates(gates);
+  for (const [id, status] of sorted) {
+    if (status === "not_started" || status === "unknown") {
+      return { id, status };
+    }
+  }
+  return null;
+}
+
+function cloneTask(task: PhaseTask | null): PhaseTask | null {
+  if (!task) return null;
+  return {
+    id: task.id,
+    title: task.title,
...
+    ...(task.validation_results ? { validation_results: task.validation_results.map(entry => ({ ...entry })) } : {})
+  };
+}
+
+function cloneTasks(tasks: PhaseTask[]): PhaseTask[] {
...
+
+  const normalizedTasks = cloneTasks(state.tasks);
+  const normalizedState: PhaseState = {
+    ...state,
...
+  const findClonedTask = (task: PhaseTask | null) => {
+    if (!task) return null;
+    return normalizedTasks.find(candidate => candidate.id === task.id) ?? cloneTask(task);
+  };
+
+  const pendingTasks = normalizedTasks.filter(task => task.status !== "complete");
+  const humanSummary = options.humanSummary ?? formatHumanSummary(state, nextAction, { validations: validations ?? undefined, uncommittedChanges });
+
+  return {
+    phaseState: normalizedState,
+    currentGate: deriveCurrentGate(normalizedState.gates),
+    nextGate: deriveNextGate(normalizedState.gates),
+    currentTask: findClonedTask(currentTask),
+    nextTask: findClonedTask(nextTask),
+    pendingTasks,
+    suggestedNextAction: { ...nextAction },
+    humanSummary,
...
new file mode 100644
index 0000000000000000000000000000000000000000..8a154f5f0b2fd50f94a301ae81c1d367fb2c8183
--- /dev/null
...
+    "contractPath": "contracts/Roadmap_workflow/03_workflow_phase3_integration_contract.json",
...
+    },
+    "tasks": [
+      { "id": "WF3-W31", "title": "Memoize workflow runtime context", "status": "complete" },
+      { "id": "WF3-W32", "title": "Expose workflow metadata", "status": "in_progress" },
+      { "id": "WF3-W33", "title": "Parity guard", "status": "pending" }
+    ]
+  },
+  "uncommittedChanges": [
+    " M src/server.ts",
...
+  ],
+  "validations": {
+    "last_run": "2025-10-14T21:40:00.000Z",
+    "lint": "pass",
+    "typecheck": "pass",
+    "test": "pass",
+    "contract_check": "pass"
...
+  "expected": {
+    "currentGate": { "id": "G2", "status": "partial" },
+    "nextGate": { "id": "G3", "status": "not_started" },
+    "currentTaskId": "WF3-W32",
+    "nextTaskId": "WF3-W33",
+    "pendingTaskIds": ["WF3-W32", "WF3-W33"],
+    "humanSummary": "Phase 19 — Autonomous Transition | Gates: G1=passed, G2=partial, G3=not_started | Validations: lint=pass, type=pass, test=pass, contract=pass | Uncommitted: 2 | Next: COMMIT_PENDING_CHANGES"
+  }
+}
diff --git a/tests/api/progress.test.ts b/tests/api/progress.test.ts
new file mode 100644
index 0000000000000000000000000000000000000000..df8f7baa975d657e53a28676a44d1be3b5a4c49f
--- /dev/null
...
+  contractPath: "contracts/Roadmap_workflow/03_workflow_phase3_integration_contract.json",
+  ledgerPath: ".automation/GATES_LEDGER.md",
+  gates: { G1: "passed", G2: "partial", G3: "not_started" },
+  tasks: [
+    { id: "WF3-W31", title: "Memoize workflow runtime context", status: "complete" },
+    { id: "WF3-W32", title: "Expose workflow metadata", status: "in_progress" },
+    { id: "WF3-W33", title: "Parity guard", status: "pending" }
...
+  action: "ADVANCE_ORCHESTRATOR_PILOT",
+  reasoning: "Runtime metadata ready for orchestrator parity checks.",
+  command: "npm test -- tests/api/progress.test.ts"
+};
+
+vi.mock("../../src/state/phaseState.js", async () => {
+  const actual = await vi.importActual<typeof import("../../src/state/phaseState.js")>(
+    "../../src/state/phaseState.js"
+  );
+  return {
+    ...actual,
+    loadPhaseState: vi.fn(async () => mockPhaseState),
+    suggestNextAction: vi.fn(() => mockNextAction),
+    determineCurrentTask: vi.fn(() => mockPhaseState.tasks[1]),
+    determineNextTask: vi.fn(() => mockPhaseState.tasks[2])
+  };
+});
+
+import { app, __progressTest, __orchestratorTest } from "../../src/server.js";
+
+const ensureWorkflow = async (sessionId: string) => {
+  await __progressTest.ensureWorkflow(sessionId);
+  __orchestratorTest.ensure(sessionId);
+};
+
+describe("progress metadata integration", () => {
+  beforeEach(async () => {
...
+  });
+
+  it("returns workflow metadata from /api/progress/snapshot/:id", async () => {
+    const sessionId = "wf-snapshot";
+    await ensureWorkflow(sessionId);
+
+    const res = await request(app).get(`/api/progress/snapshot/${sessionId}`);
+    expect(res.status).toBe(200);
+    expect(res.body.workflowMetadata).toBeDefined();
+
+    const metadata = res.body.workflowMetadata;
+    expect(metadata.humanSummary).toContain("Phase 19 — Autonomous Transition");
+    expect(metadata.humanSummary).toContain(mockNextAction.action);
+    expect(metadata.currentTask?.id).toBe("WF3-W32");
+    expect(metadata.pendingTasks.map((task: { id: string }) => task.id)).toContain("WF3-W33");
+    expect(metadata.suggestedNextAction.action).toBe(mockNextAction.action);
+  });
+
+  it("attaches workflow metadata to cached progress entries and snapshots", async () => {
+    const sessionId = "wf-cache";
+    await ensureWorkflow(sessionId);
+
+    __progressTest.update(sessionId, "generating", 42, { checkpoint: false });
+
+    const stored = __progressTest.get(sessionId);
+    expect(stored?.workflowMetadata).toBeDefined();
+    expect(stored?.workflowMetadata?.suggestedNextAction.action).toBe(mockNextAction.action);
+
+    const snapshot = __progressTest.snapshot(sessionId);
+    expect(snapshot?.workflowMetadata?.humanSummary).toContain("Phase 19 — Autonomous Transition");
+    expect(snapshot?.workflowMetadata?.humanSummary).toContain(mockNextAction.action);
+    expect(snapshot?.workflowMetadata?.currentGate?.id).toBe("G2");
+  });
+});
diff --git a/tests/state/phaseState.test.ts b/tests/state/phaseState.test.ts
index 97587c7f6ee58c0482dc54516feffc28642812b9..de31aff32c44ffffa42685e5c3af1dc9a55ed28f 100644
...
@@ -56,27 +59,65 @@ describe("phaseState shared module", () => {
...
+
+  it("aligns runtime workflow metadata with CLI snapshot fixture", async () => {
+    const fixturePath = path.resolve("tests/api/__fixtures__/workflowProgress.json");
...
+      timestamp: number;
+      expected: {
+        currentGate: { id: string; status: string };
+        nextGate: { id: string; status: string } | null;
...
+    };
+
+    const metadata = buildWorkflowMetadata(fixture.phaseState, fixture.nextAction, {
+      validations: fixture.validations,
+      uncommittedChanges: fixture.uncommittedChanges,
+      timestamp: fixture.timestamp
+    });
+
+    expect(metadata.phaseState).toEqual(fixture.phaseState);
+    expect(metadata.currentGate).toEqual(fixture.expected.currentGate);
+    expect(metadata.nextGate).toEqual(fixture.expected.nextGate);
+    expect(metadata.currentTask ? metadata.currentTask.id : null).toBe(fixture.expected.currentTaskId);
+    expect(metadata.nextTask ? metadata.nextTask.id : null).toBe(fixture.expected.nextTaskId);
+    expect(metadata.pendingTasks.map(task => task.id)).toEqual(fixture.expected.pendingTaskIds);
+    expect(metadata.suggestedNextAction).toEqual(fixture.nextAction);
+    expect(metadata.uncommittedChanges).toEqual(fixture.uncommittedChanges);
+    expect(metadata.validations).toEqual(fixture.validations);
+    expect(metadata.humanSummary).toBe(fixture.expected.humanSummary);
+    expect(metadata.computedAt).toBe(fixture.timestamp);
+  });
 });
